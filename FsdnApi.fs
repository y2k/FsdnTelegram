...
let execute (req: Request) =
    async {
        ...
        msg.Headers.UserAgent.ParseAdd
        ...
        return
            ...
            | _ ->
                ...
                let xs =
                    JsonSerializer().Deserialize<JsonRespones>(reader)
                ...
                |> List.map
                    (fun x ->
                        { url = x.api.link
                          clazz =
                              x.api.name.``namespace``
                              + "."
                              + x.api.name.class_name
